<template>
  ...
  <div class="columns is-multiline">
    <div class="column is-3" v-for="item in lists" :key="item.id" @click="open_detail(item)">
      <div style="width=100%; height: 200px; background:green ; margin-top:25px ; ">
        <div>
          {{ item.title }}
          <img :src="item.img_cover" alt="" style="width: 100%; height: 200px" />
        </div>
      </div>
    </div>
  </div>
  ...
  <div v-bind:class="{ modal: true, 'is-active': modal_open }">
    ...
    <div class="modal-card">
      <header class="modal-card-head">
        <p class="modal-card-title">แก้ไขวิดีโอ</p>
        <button class="delete" aria-label="close" @click="close_modal"></button>
      </header>
      <section class="modal-card-body">
        <!-- Content ... -->
        <div class="field">
          <label class="label">ชื่อวิดีโอ</label>
          <div class="control">
        ...
        <div class="field">
          <label class="label">ลิ้งค์รูปภาพปก</label>
          <div class="control">
            <input class="input" type="text" placeholder="Text input" v-model="detail.img_cover" />
          </div>
        </div>

        <div class="field">
          <label class="label">ลิ้งค์วิดีโอ</label>
          <div class="control">
            <input class="input" type="text" placeholder="Text input" v-model="detail.vdo_link" />
          </div>
        </div>
      </section>
      <footer class="modal-card-foot">
        ...
        <button class="button is-danger" @click="delete_vdo">ลบข้อมูล</button>
      </footer>
    </div>
  </div>
  ...
  <div v-bind:class="{ modal: true, 'is-active': modal_create_open }">
    ...
    <div class="modal-card">
      <header class="modal-card-head">
        <p class="modal-card-title">สร้างวิดีโอ</p>
      ...
      <section class="modal-card-body">
        ...
        <div class="field">
          <label class="label">ลิ้งค์รูปภาพปก</label>
          <div class="control">
            <input class="input" type="text" placeholder="Text input" v-model="detail_create.img_cover" />
          </div>
        </div>
        ...
        <div class="field">
          <label class="label">ลิ้งค์วิดีโอ</label>
          <div class="control">
            <input class="input" type="text" placeholder="Text input" v-model="detail_create.vdo_link" />
          </div>
        </div>
      </section>
      <footer class="modal-card-foot">
